...
d3.csv("data/merge.csv").then(function(data) {
    // Process data to create a structure for heatmap data
    const allYearData = {};
    data.forEach(d => {
        if (d.track_genre === 'pop') { // Filter by 'pop' genre
            const year = new Date(d.release_date).getFullYear();
            const country = d.artist_country;
            if (!allYearData[year]) allYearData[year] = {};
            if (!allYearData[year][country]) allYearData[year][country] = 0;
            allYearData[year][country] += 1; // Increment track count
        }
    });

    // Rest of your world map code here, using allYearData
...
d3.json("https://d3js.org/world-110m.v1.json").then(world => {
    ...
    function updateHeatmap(year) {
        countries.transition()
            .duration(500)
            .attr("fill", d => {
                const countryData = allYearData[year] || {};
    ...
    // Initialize the map with the first year in the dataset
    updateHeatmap(Object.keys(allYearData)[0]);
});   
});
